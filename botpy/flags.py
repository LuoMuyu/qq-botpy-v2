# -*- coding: utf-8 -*-
"""事件订阅 Intents 与子频道权限 Permission。

按照最新官方文档，新增 ``GROUP_MEMBER_EVENT (1<<24)`` 事件类型
（群成员加入/退出事件），其余 intents 位与原版保持一致。
"""
...
class Intents(BaseFlags):
    """
    事件订阅 intentions，对应官方文档的 intents 位：
    ...
    guild_messages                1 << 9     频道消息事件（仅私域机器人）
    guild_message_reactions       1 << 10    频道消息表情表态事件
    direct_message                1 << 12    频道私信事件
    open_forum_event              1 << 18    开放论坛事件（公域）
    audio_or_live_channel_member  1 << 19    音视频/直播子频道成员进出事件
    group_member_event            1 << 24    群成员加入/退出事件（新增）
    public_messages               1 << 25    群聊/C2C 事件（GROUP_AND_C2C_EVENT）
    interaction                   1 << 26    按钮互动事件
    message_audit                 1 << 27    消息审核事件
    forums                        1 << 28    论坛事件（仅私域机器人）
    audio_action                  1 << 29    音频事件
    public_guild_messages         1 << 30    频道公域消息事件
    ============================  =========  ==================================
    """
    ...
    def public_messages(self):
        """:class:`bool`: 是否打开公域群/C2C消息事件的监听（GROUP_AND_C2C_EVENT 1<<25）.

        通过增加`client`的`on_xx`事件可以获取事件下发的数据:

        - :func:`on_group_at_message_create`            // 当收到群@机器人的消息时
        - :func:`on_group_message_create`               // 群消息全量模式，群内所有消息（需开通）
        ...
        - :func:`on_group_join_request`                 // 用户申请加群事件（需机器人为群管理员）
    ...
    def group_member_event(self):
        """:class:`bool`: 是否打开群成员事件的监听（GROUP_MEMBER_EVENT 1<<24，新增）.

        通过增加`client`的`on_xx`事件可以获取事件下发的数据:

        - :func:`on_group_member_add`                   // 有新成员加入群聊时
        - :func:`on_group_member_quit`                  // 有成员退出群聊时
