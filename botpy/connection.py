"""Client 的 WebSocket 连接会话与事件状态处理。

相比原版 botpy，新增以下事件解析器（对应最新官方文档）：
  - group_message_create:  群消息全量模式（GROUP_MESSAGE_CREATE）
  - group_join_request:    用户申请加群（GROUP_JOIN_REQUEST）
  - group_member_add / group_member_quit: 群成员加入/退出（GROUP_MEMBER_ADD/QUIT）
"""
...
from .manage import C2CManageEvent, GroupManageEvent, GroupJoinRequest, GroupMemberEvent
...
class ConnectionState:
    ...
    # botpy.flags.Intents.public_messages（GROUP_AND_C2C_EVENT 1<<25）
    ...
    def parse_group_message_create(self, payload):
        # 群消息全量模式（GROUP_MESSAGE_CREATE）：
        # 机器人开启"接收所有消息"后，群内每一条消息（不限于@机器人）都会推送，
        # 字段含义与 GROUP_AT_MESSAGE_CREATE 完全一致
        _message = GroupMessage(self.api, payload.get("id", None), payload.get("d", {}))
    ...
    def parse_group_join_request(self, payload):
        # 用户申请加群事件（GROUP_JOIN_REQUEST），需机器人为群管理员
        _event = GroupJoinRequest(self.api, payload.get("id", None), payload.get("d", {}))
    ...
    # botpy.flags.Intents.group_member_event（GROUP_MEMBER_EVENT 1<<24，新增）
    ...
    def parse_group_member_quit(self, payload):
        _event = GroupMemberEvent(self.api, payload.get("id", None), payload.get("d", {}))
        self._dispatch("group_member_quit", _event)
